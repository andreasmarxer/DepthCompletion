function [xy_bb_left, xy_bb_right] = getLeftRightLinesInBBwGravity(lines, x1, x2, y1, y3, debug)
...
for k = 1:length(lines)
    ...
    if lines(k).angle2z < 4
        ...
        if lines(k).point1(1) < x_bb_2_3_left && lines(k).point2(1) < x_bb_2_3_left
            ...
            if (len > 1.1*max_len_left) %&& (len > round(1/5*(y3-y1)))
                max_len_left = len;
        ...
        elseif lines(k).point1(1) > x_bb_1_3_right && lines(k).point2(1) > x_bb_1_3_right
            len = norm(lines(k).point1 - lines(k).point2);
            if (len > 1.1*max_len_right) %&& (len > round(1/5*(y3-y1)))
                max_len_right = len;
